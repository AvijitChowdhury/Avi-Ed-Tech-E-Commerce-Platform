...
export default function ProductCard({ p }: { p: ProductLike }) {
  ...
  return (
    <div className="group card-elevated rounded-2xl overflow-hidden transition-all hover:-translate-y-1 hover:glow-primary">
      <Link to="/products/$slug" params={{ slug: p.slug }} className="block aspect-[4/3] overflow-hidden bg-muted relative">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          <span className="bg-background/80 backdrop-blur px-2 py-1 rounded-md text-xs font-medium flex items-center gap-1">
            <Icon className="h-3 w-3" />
            {p.type === "lab" ? "Lab" : "Course"}
          </span>
          {compare && compare > price && (
            <span className="gradient-primary-bg text-primary-foreground px-2 py-1 rounded-md text-xs font-bold">
      ...
      <div className="p-4 flex flex-col gap-2">
        ...
        <Link to="/products/$slug" params={{ slug: p.slug }} className="font-semibold leading-tight line-clamp-2 hover:text-primary transition-colors">
          {p.title}
        </Link>
        {p.short_description && (
        ...
        <div className="flex items-center gap-1 text-xs">
          ...
          <span className="font-medium">{Number(p.rating ?? 4.8).toFixed(1)}</span>
          <span className="text-muted-foreground">({p.rating_count ?? 0})</span>
        </div>
        <div className="flex items-end justify-between mt-1">
          <div>
            <span className="font-display text-xl font-bold gradient-text">{money(price)}</span>
            {compare && compare > price && (
          ...
          <Button
            size="sm"
            className="gradient-primary-bg text-primary-foreground hover:opacity-90"
            onClick={() => {
